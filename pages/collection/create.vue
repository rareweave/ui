<template>
    <form v-if="!uploading" class="h-full-navbared w-full flex flex-col items-center justify-center font-mono" @submit.prevent="Create">
      <h1 class="text-3xl text-center"> Create <p class="rareweave-font">Collection</p>
      </h1>
      <div class="form-control mb-2 pb-2 px-2">
        <label class="label">
          <span class="label-text">Collection name</span>
        </label>
        <input v-model="title" required type="text" maxlength="40" placeholder="RareWeave Collection" class="input input-bordered" />
        <label class="label">
          <span class="label-text">Describe your Collection</span>
        </label>
        <textarea placeholder="Super Awesome NFT's!" v-model="description" class="textarea textarea-bordered textarea-xs w-full max-w-xs"></textarea>
        <label class="label">
          <span class="label-text">Tags</span>
        </label>
        <input v-model="tags" required type="text" maxlength="40" placeholder="doge rare cool..." class="input input-bordered" />
        <button type="submit" class="btn btn-lg py-3 amazing-button rounded-lg min-h-0 h-auto my-2"> Create! </button>
      </div>
    </form>
    <div v-else class="h-full-navbared w-full flex flex-col items-center justify-center font-mono">
      ...
      <p class="text-sm text-center text-zinc-400"> Wait few minutes for your Collection to be mined. <br /> May take a few minutes... </p>
    </div>
  </template>
  <script setup>
