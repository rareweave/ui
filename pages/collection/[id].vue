<template>
  ...
  >
    <div
      class="pt-4 px-4 my-4 ml-4 bg-black bg-opacity-30 flex flex-col items-center"
    >
    ...
    <div class="flex flex-row flex-wrap items-start">
      ...
      >
        <template v-for="nft in nfts.result">
          <NftCard
            v-if="state.items.includes(nft.contractTxId)"
...
async function refreshResults() {
  nfts.value = await $fetch(
    `https://prophet.rareweave.store/nfts?search=${searchCondition.value}${
