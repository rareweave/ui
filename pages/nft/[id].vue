<template>
    <div class="h-full-navbared w-full flex flex-col items-center justify-center">
        <div class="m-2 rounded mt-6">
            <img v-if="nftState?.contentType?.startsWith('image')"
        ...
        <div class="my-2 w-max min-w-[14rem]">
            <div
                ...
                Owner: <div v-if="account && account.addr && account.addr == nftOwner.addr"
            ...
            <div
                class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between">
                Minter: <div v-if="account && account.addr && account.addr == nftMinter.addr"
                    class="text-sm rounded-xl p-2 ml-2 bg-zinc-700 flex-row flex items-center ">You</div>
            ...
            <div
                class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between">
                For sale: <template v-if="isNftOwner">
                    <input type="checkbox" :class="['toggle', (nftState.forSale ? 'toggle-success' : 'toggle-error')]"
            ...
            <template v-if="nftState.forSale">
                <div
                    class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between">
                    Price: <template v-if="isNftOwner">
                ...
                <div
                    class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between">
                    Royalty: <div class="p-2 bg-zinc-700 text-sm">{{ nftRoyalty }}%</div>
                </div>
            </template>
            <button v-if="isNftOwner"
                :class="['btn', 'amazing-button2', 'rounded-md', 'w-full', 'my-2', , changed ? '' : 'btn-disabled']"
                :disabled="!changed" @click="saveChangesToNft">Save changes</button>
        ...
        <div class="modal">
            <div class="modal-box relative flex flex-col " v-if="buyStatus != 3">
                <label for="buy-modal" class="btn btn-sm  absolute right-2 top-2"
                    v-if="buyStatus == 0 || buyStatus == 3">✕</label>
                <h3 class="font-bold text-lg text-center">Buy "{{ nftState.name }}"</h3>
                <ul class="steps steps-vertical mt-4 ml-4">
                    <li class="step step-primary">
                        <button @click="payRoyalty" v-if="!payingRoyalty && !isBuying"
                            class="btn amazing-button2 rounded-lg btn-sm">Pay royalty ({{ nftPrice * (nftRoyalty / 100) }}
                            AR)</button>
                    ...
                    <li :class="{ step: true, 'step-primary': isBuying }">
                        <button :disabled="!isBuying" v-if="!isSomeoneElseBuying && buyStatus != 2"
                            class="btn amazing-button rounded-lg btn-sm" @click="finalizeBuy">Finalize buy ({{ nftPrice }}
                            AR)</button>
                        <span v-else-if="buyStatus == 2" class="text-lg">Finalizing buy... </span>
                        <button v-else class="btn btn-error btn-outline rounded-lg btn-sm">Someone else did reserve this NFT
                            before you</button>
                    </li>
                </ul>
