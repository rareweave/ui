<template>
    <div class="h-full-navbared w-full flex flex-col items-center justify-center">
      ...
      <template v-if="account && account.addr && account.addr == nftOwner.addr">
        <textarea class="text-md textarea  font-mono m-2 max-w-[32rem] w-full overflow-auto transition-none textarea-bordered rounded-md" v-model="nftState.description"></textarea>
      </template>
      <template v-else>
        <p class="text-md text-center font-mono p-2 max-w-[32rem]" v-if="nftState.description">{{ nftState.description
              }}
        </p>
        <p class="text-md text-center font-mono text-zinc-500" v-else>[This NFT has no description]</p>
      </template>
      <div class="my-2 w-max min-w-[14rem]">
        <div class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between"> Owner: <div v-if="account && account.addr && account.addr == nftOwner.addr" class="text-sm rounded-xl p-2 ml-2 bg-zinc-700 flex-row flex items-center ">You</div>
          <NuxtLink :to="'/profile/' + nftOwner.addr" v-else class="text-sm rounded-xl p-1 ml-2 bg-zinc-700 flex-row flex items-center ">
            <img :src="nftOwner.profile.avatarURL" class="w-8 rounded-xl mr-2" />
            {{ nftOwnerANS || nftOwner.handle }}
          </NuxtLink>
        </div>
        <div class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between"> Minter: <div v-if="account && account.addr && account.addr == nftMinter.addr" class="text-sm rounded-xl p-2 ml-2 bg-zinc-700 flex-row flex items-center ">You</div>
          <NuxtLink :to="'/profile/' + nftMinter.addr" v-else class="text-sm rounded-xl p-1 ml-2 bg-zinc-700 flex-row flex items-center ">
            <img :src="nftMinter.profile.avatarURL" class="w-8 rounded-xl mr-2" />
            {{ nftMinterANS || nftMinter.handle }}
          </NuxtLink>
        </div>
        <div class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between"> For sale: <template v-if="isNftOwner">
            <input type="checkbox" :class="['toggle', (nftState.forSale ? 'toggle-success' : 'toggle-error')]" v-model="nftState.forSale" />
          </template>
          <template v-else>
            <div v-if="nftState.forSale" class="p-2 bg-green-600 text-sm">Yes</div>
            <div v-else class="p-2 bg-red-600 text-sm">No</div>
          </template>
        </div>
        <template v-if="nftState.forSale">
          <div class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between"> Price: <template v-if="isNftOwner">
              <div class="p-1 pr-2 bg-zinc-700 text-sm">
                <input v-model="nftPrice" type="number" step="0.1" class="input text-start  appearance-[textfield] w-14 rounded-none px-1 input-sm bg-transparent" placeholder="Price" /> AR
              </div>
            </template>
            <div v-else class="p-2 bg-zinc-700 text-sm">{{ nftPrice }} AR</div>
          </div>
          <div class="rounded-none w-full bg-zinc-900 border-b-2 border-black text-white p-2 flex flex-row items-center justify-between"> Royalty: <div class="p-2 bg-zinc-700 text-sm">{{ nftRoyalty }}%</div>
          </div>
        </template>
        <button v-if="isNftOwner" :class="['btn', 'amazing-button2', 'rounded-md', 'w-full', 'my-2', , changed ? '' : 'btn-disabled']" :disabled="!changed" @click="saveChangesToNft">Save changes</button>
        ...
        <div class="modal-box relative flex flex-col " v-if="buyStatus != 3">
          ...
          <ul class="steps steps-vertical mt-4 ml-4">
            <li class="step step-primary">
              <button @click="payRoyalty" v-if="!payingRoyalty && !isBuying" class="btn amazing-button2 rounded-lg btn-sm">Pay royalty ({{ nftPrice * (nftRoyalty / 100) }} AR)</button>
            ...
            <li :class="{ step: true, 'step-primary': isBuying }">
              <button :disabled="!isBuying" v-if="!isSomeoneElseBuying && buyStatus != 2" class="btn amazing-button rounded-lg btn-sm" @click="finalizeBuy">Finalize buy ({{ nftPrice }} AR)</button>
              <span v-else-if="buyStatus == 2" class="text-lg">Finalizing buy... </span>
              <button v-else class="btn btn-error btn-outline rounded-lg btn-sm">Someone else did reserve this NFT before you</button>
            </li>
          </ul>
      ...
      <div class="modal">
        <div class="modal-box relative flex flex-col " v-if="!isSomeoneElseBuying">
          ...
          <form class="modal-action" @submit.prevent="transfer">
            <input v-model="transferRecipient" class="input input-bordered w-full rounded-lg p-2" type="text" required placeholder="Recipient's arweave address" />
  ...
  <script setup>
    ...
    let changed = computed(() => {
      let ch = nftPrice.value != parseFloat(parseFloat(arweave.ar.winstonToAr(nftStateOrig.value.price)).toFixed(3)) || nftStateOrig.value.description != nftState.value.description || nftStateOrig.value.forSale != nftState.value.forSale
      return ch
    })
    ...
    async function saveChangesToNft() {
      ...
      nftStateOrig.value = JSON.parse(JSON.stringify({
        ...nftState.value,
        price: arweave.ar.arToWinston(nftPrice.value)
      }))
      console.log(nftPrice.value != parseFloat(parseFloat(arweave.ar.winstonToAr(nftStateOrig.value.price)).toFixed(3)), nftStateOrig.value.description != nftState.value.description, nftStateOrig.value.forSale != nftState.value.forSale)
    }
    ...
    async function payRoyalty() {
      ...
      try {
        ...
        let tx = await arweave.createTransaction({
          ...
          quantity: (parseInt(arweave.ar.arToWinston(nftPrice.value)) * nftState.value.royalty).toString(),
          reward: feeEstimate
        })
