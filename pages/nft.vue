        ...
        <div class="modal">
            <div class="modal-box relative flex flex-col " v-if="buyStatus != 3">
                ...
                <ul class="steps steps-vertical mt-4 ml-4">
                    <li class="step step-primary"><button @click="payRoyalty" v-if="!payingRoyalty && !isBuying"
                            class="btn amazing-button2 rounded-lg btn-sm">Pay royalty
                            ({{ nftPrice * (nftRoyalty / 100) }} AR)</button>
                    ...
                    <li :class="{ step: true, 'step-primary': isBuying }">
                        <button :disabled="!isBuying" v-if="!isSomeoneElseBuying && buyStatus != 2"
                            class="btn amazing-button rounded-lg btn-sm" @click="finalizeBuy">Finalize buy
                            ({{ nftPrice }} AR)</button>
                        <span v-else-if="buyStatus == 2" class="text-lg">Finalizing buy... </span>
                        <button v-else class="btn btn-error btn-outline rounded-lg btn-sm">Someone else did reserve this
                            NFT
                            before you</button>
                    </li>
                </ul>
            ...
            <div class="modal-box relative flex flex-col items-center" v-else>
                ...
                <img src="~/assets/image.svg" class="w-2/3 m-2">
...
async function saveChangesToNft() {
    ...
    nftStateOrig.value = JSON.parse(JSON.stringify({ ...nftState.value, price: arweave.ar.arToWinston(nftPrice.value) }))
    console.log(nftPrice.value != parseFloat(parseFloat(arweave.ar.winstonToAr(nftStateOrig.value.price)).toFixed(3))
        , nftStateOrig.value.description != nftState.value.description
        , nftStateOrig.value.forSale != nftState.value.forSale)
}
