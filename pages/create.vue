<template>
  <form v-if="!uploading" class="h-full-navbared w-full flex flex-col items-center justify-center font-mono" @submit.prevent="mint">
    ...
    <div class="flex items-center justify-center w-full md:w-96 m-4 pt-4 px-4">
      <label for="dropzone-file" class="flex flex-col items-center justify-center w-full min-h-64 border-2 border-dashed rounded-lg cursor-pointer bg-base-300 hover:bg-base-200 border-zinc-800">
        <div class="flex flex-col items-center justify-center pt-5 pb-6">
          <svg v-if="!imageObjectUrl" aria-hidden="true" class="w-10 h-10 mb-3 mt-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          ...
          <template v-else>
            ...
            <video v-else-if="fileMeta?.type?.startsWith('video')" autoplay muted controls>
              <source :src="imageObjectUrl" :type="fileMeta.type"> Your browser does not support the video tag.
            </video>
          </template>
    ...
    <div class="form-control mb-2 pb-2 px-2 ">
      ...
      <input v-model="title" required type="text" maxlength="40" placeholder="Name for your NFT" class="input input-bordered" />
      <label class="label">
        <span class="label-text">Describe your NFT</span>
      </label>
      <textarea placeholder="NFT description" v-model="description" class="textarea textarea-bordered textarea-xs w-full max-w-xs"></textarea>
      <label class="label">
  ...
  <div v-else class="h-full-navbared w-full flex flex-col items-center justify-center font-mono">
    ...
    <p class="text-sm text-center text-zinc-400">Wait few minutes for your NFT to be mined. <br /> You may take coffee, sit back and relax. It will take at least 1 Arweave block (~2 minutes). </p>
  </div>
</template>
<script setup>
  ...
  async function mint() {
    ...
    if (collectionId.value != "") {
      let collectionContract = account.value ? warp.contract(collectionId.value).setEvaluationOptions({
        remoteStateSyncSource: "https://prophet.rareweave.store/contract",
        remoteStateSyncEnabled: true,
        unsafeClient: "allow",
        allowBigInt: true,
        waitForConfirmation: false,
      }).connect("use_wallet") : warp.contract(collectionId.value).setEvaluationOptions({
        remoteStateSyncSource: "https://prophet.rareweave.store/contract",
        remoteStateSyncEnabled: true,
        unsafeClient: "allow",
        allowBigInt: true,
